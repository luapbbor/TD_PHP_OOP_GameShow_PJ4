...
public $selected = [];
// Random phrases to select generate
...
public function addPhraseToDisplay() {
    // puts the phrase to lowercase
    $currentPhraseLower = strtolower($this->currentPhrase);
    // Puts each letter from currentphase into an array
    $letters = str_split($currentPhraseLower);
    // Loop through each of those letters 
    foreach($letters as $letter){
        
        echo "<ul>";
        // If the letter is empty, add the space class
        if ($letter == " "){
            echo "<li class='hide space'></li>";
        } else {
            // If the letter is in the selected array add the show class, else add the hide class
